...
void CreateCubes::createVoxelspace(float spaceLength, float voxelSize,QStringList m_filepathes)
{    
...
bool CreateCubes::checkDuplicateFace(int i, int number_x, int number_y, int number_z)
{
    ...
    if (i==0 && number_z > 0){
        if(voxelizer.voxelspace[number_x][number_y][number_z - 1].getStatus()== 'O'){
            return true;
        }
    }
    // number_y + 1 has overlapping face
    else if (i==1 && number_y < n_voxel_in_axis - 1){
        if(voxelizer.voxelspace[number_x][number_y + 1][number_z].getStatus()== 'O'){
            return true;
        }
    }
    // number_x + 1 has overlapping face
    else if (i==2 && number_x < n_voxel_in_axis - 1){
        if(voxelizer.voxelspace[number_x + 1][number_y][number_z].getStatus()== 'O'){
            return true;
        }
    }
    // number_x - 1 has overlapping face
